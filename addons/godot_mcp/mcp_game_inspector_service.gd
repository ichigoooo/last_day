...
func _cmd_execute_script(params: Dictionary) -> void:
	...
	# Top-level `func run(...)` is special: its body becomes the MCP entrypoint body only,
	# otherwise we'd emit both user `run` and wrapper `run` → duplicate function error.
	...
	while i < raw_lines.size():
		...
		if not line.begins_with("\t") and not line.begins_with(" ") and line.begins_with("func "):
			...
			var is_reserved_run := false
			if stripped.begins_with("func run"):
				var after_run: String = stripped.substr(8).strip_edges()
				is_reserved_run = after_run.is_empty() or after_run.begins_with("(") or after_run.begins_with(":")
			if is_reserved_run:
				i += 1
				while i < raw_lines.size():
					...
					if next_line.is_empty() or next_line.begins_with("\t"):
						if next_line.begins_with("\t"):
							body_lines.append(next_line.substr(1))
						else:
