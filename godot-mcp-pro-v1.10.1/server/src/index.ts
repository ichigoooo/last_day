...
async function main() {
  ...
  try {
    ...
    if (
      ...
      console.error(
        "[MCP] Port in use: another godot-mcp-pro instance is already running (e.g. multiple Cursor MCP sessions). Keep a single session or run: pkill -f 'godot-mcp-pro-v1.10.1/server/build/index.js'"
      );
      process.exit(1);
    }
    throw err;
